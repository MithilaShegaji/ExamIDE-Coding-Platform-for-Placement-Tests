...
// Your original controller logic - ONLY performance optimized
// Keep all your existing logic, just make it faster

// Memory-Efficient Controller - Prevents JavaScript heap overflow
// Your original logic but with memory management

// Simple Paginated Controller - Only process data for current page
// Your original logic but with true pagination

// Bulletproof Simple Controller - Your original logic with minimal changes

// Fast Paginated Controller - Only process MCQ scores for current page
...
//         if (isAjaxDataRequest) {
//             // Handle AJAX request - return JSON data (FAST PAGINATION)
//             return await handleCandidatesDataRequestFast(req, res, examId);
//         }
        
//         // Handle regular page request - return HTML (UNCHANGED)
//         return await handleCandidatesPageRequest(req, res, examId);
...
// // Helper function for page requests (UNCHANGED)
// async function handleCandidatesPageRequest(req, res, examId) {
//     const exam = await Exam.findById(examId)
//         .populate('mcqQuestions')
//         .populate('codingQuestions');
...
//             Submission.countDocuments({ exam: examId }),
//             EvaluationResult.countDocuments({ examId: examId })
//         ]);
...
// // FAST data request handler - TRUE pagination
// async function handleCandidatesDataRequestFast(req, res, examId) {
//     console.log(`[${new Date().toISOString()}] Starting FAST processing for exam ${examId}`);
//     const startTime = Date.now();
...
//         const skip = (page - 1) * limit;
//         console.log(`Processing page ${page}, limit ${limit}, skip ${skip}`);
...
//         console.log(`Exam type: ${exam.questionType}, hasMCQ: ${hasMCQ}, hasCoding: ${hasCoding}`);
...
//             const codingEvaluation = await EvaluationResult.findOne({
//                 examId: exam._id
//             }).select('maxPossibleScore').lean();
//             maxCodingScore = codingEvaluation ? (codingEvaluation.maxPossibleScore || 0) : 0;
//         }
        
//         console.log(`Max scores: MCQ=${maxMCQScore}, Coding=${maxCodingScore}`);
        
//         // STEP 1: Get total count for pagination
//         console.log('Getting total count...');
//         const totalCount = await getTotalCandidatesCount(examId, search, departmentFilter);
//         console.log(`Total candidates: ${totalCount}`);
        
//         // STEP 2: Get ONLY current page submissions with filters
//         console.log('Getting page submissions...');
//         const pageSubmissions = await getPageSubmissionsOptimized(examId, search, departmentFilter, skip, limit);
//         console.log(`Page submissions: ${pageSubmissions.length}`);
        
//         // STEP 3: Get ONLY current page active sessions
//         console.log('Getting page active sessions...');
//         const submittedStudentIds = pageSubmissions.map(s => s.student._id.toString());
//         const pageActiveSessions = await getPageActiveSessionsOptimized(examId, search, departmentFilter, submittedStudentIds, skip, limit);
//         console.log(`Page active sessions: ${pageActiveSessions.length}`);
        
//         // STEP 4: Process ONLY current page candidates (FAST!)
//         console.log('Processing ONLY current page candidates...');
//         const pageCandidates = await processCurrentPageCandidates(
//             pageSubmissions, 
//             pageActiveSessions, 
//             hasMCQ, 
//             hasCoding, 
...
//         console.log(`Processed page candidates: ${pageCandidates.length}`);
        
//         // Apply status filter
//         let filteredCandidates = pageCandidates;
//         if (statusFilter) {
//             filteredCandidates = applyStatusFilter(pageCandidates, statusFilter);
//             console.log(`After status filter: ${filteredCandidates.length}`);
//         }
        
//         // Sort candidates
//         filteredCandidates.sort(candidateComparator);
        
//         const totalPages = Math.ceil(totalCount / limit);
        
//         const endTime = Date.now();
//         console.log(`[${new Date().toISOString()}] FAST: Processed ${filteredCandidates.length} candidates in ${endTime - startTime}ms`);
...
//     } catch (error) {
//         console.error(`[${new Date().toISOString()}] Error in fast handler:`, error);
//         return res.status(500).json({ 
//             success: false, 
//             message: 'Error processing candidates'
//         });
//     }
// }

// // Get total candidates count (submissions + active non-submitted users)
// async function getTotalCandidatesCount(examId, search, departmentFilter) {
//     let studentFilter = {};
...
//         const matchingStudents = await mongoose.model('Student')
//             .find(studentFilter)
//             .select('_id')
//             .lean();
...
//         submissionQuery.student = { $in: matchingStudentIds };
//     }
//     const submissionsCount = await Submission.countDocuments(submissionQuery);
    
//     // Count active sessions (non-submitted users)
//     const submittedUserIds = await Submission.find({ exam: examId }).select('student').lean();
//     const submittedIds = submittedUserIds.map(s => s.student);
    
//     let activeQuery = { 
//         examId: examId,
//         userId: { $nin: submittedIds }
//     };
//     if (matchingStudentIds.length > 0) {
//         activeQuery.userId = { 
//             $nin: submittedIds,
...
//         };
//     }
//     const activeCount = await ActivityTracker.countDocuments(activeQuery);
    
//     return submissionsCount + activeCount;
// }

// // Get submissions for current page ONLY
// async function getPageSubmissionsOptimized(examId, search, departmentFilter, skip, limit) {
//     let submissionQuery = { exam: examId };
...
//     // Get ONLY the page we need
//     return await Submission.find(submissionQuery)
//         .populate('student', 'USN email Department Semester Rollno _id fname lname')
//         .sort({ submittedAt: -1 })
//         .skip(skip)
//         .limit(limit)
...
// // Get active sessions for current page ONLY
// async function getPageActiveSessionsOptimized(examId, search, departmentFilter, submittedStudentIds, skip, limit) {
//     let activeQuery = { 
//         examId: examId,
//         userId: { $nin: submittedStudentIds }
//     };
    
//     return await ActivityTracker.find(activeQuery)
//         .populate({
//             path: 'userId',
//             select: 'USN email Department Semester Rollno _id fname lname',
//             match: search || departmentFilter ? {
//                 $and: [
//                     search ? {
//                         $or: [
//                             { USN: { $regex: search, $options: 'i' } },
...
//         .limit(limit)
//         .lean()
//         .then(sessions => sessions.filter(s => s.userId));
// }

// // Process ONLY current page candidates (FAST!)
// async function processCurrentPageCandidates(pageSubmissions, pageActiveSessions, hasMCQ, hasCoding, maxMCQScore, maxCodingScore, examId) {
//     const candidates = [];
    
//     // Process submitted students (ONLY current page)
//     if (pageSubmissions.length > 0) {
//         console.log(`Processing MCQ scores for ${pageSubmissions.length} submissions (current page only)...`);
        
//         const studentIds = pageSubmissions.map(s => s.student._id.toString());
        
//         // Get evaluations for current page only
//         const evaluationMap = new Map();
//         if (hasCoding) {
//             const evaluations = await EvaluationResult.find({
//                 examId: examId,
//                 userId: { $in: studentIds }
//             }).select('userId totalScore').lean();
...
//         // Get activity status for current page only
//         const activityMap = new Map();
//         const activitySessions = await ActivityTracker.find({
//             examId: examId,
//             userId: { $in: studentIds }
//         }).select('userId status lastPingTimestamp').lean();
        
//         activitySessions.forEach(session => {
//             activityMap.set(session.userId.toString(), session);
//         });
        
//         // Process MCQ scores for current page ONLY (much faster!)
...
//                     if (report && report.score) {
//                         mcqScore = hasCoding ? 
//                             (report.score.mcq?.obtained || 0) : 
//                             (report.score.obtained || 0);
//                     }
//                 } catch (error) {
//                     console.error(`Error getting MCQ score for ${submission._id}:`, error);
//                     mcqScore = 0;
//                 }
//             }
...
//             const activityStatus = activity ? activity.status : 'offline';
//             const lastActive = activity ? activity.lastPingTimestamp : null;
...
//         console.log(`Processed ${pageSubmissions.length} MCQ scores in current page`);
//     }
    
//     // Process active sessions (current page only)
//     pageActiveSessions.forEach(session => {
//         if (session.userId) {
//             candidates.push({
...
//                 submittedAt: null,
//                 activityStatus: session.status,
//                 lastActive: session.lastPingTimestamp,
//                 hasSubmitted: false
//             });
//         }
//     });
...
// // Helper functions
// function applyStatusFilter(candidates, statusFilter) {
//     switch (statusFilter) {
//         case 'submitted':
//             return candidates.filter(c => c.hasSubmitted);
//         case 'active':
//             return candidates.filter(c => c.activityStatus === 'active' && !c.hasSubmitted);
//         case 'inactive':
//             return candidates.filter(c => c.activityStatus !== 'active' && !c.hasSubmitted);
//         default:
//             return candidates;
