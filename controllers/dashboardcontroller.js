...
//     });
//     console.log("partial submittion");
    
//     // Calculate score (if exam has automatic grading enabled)
//     if (studentId) {
//       let totalScore = 0;
//       console.log("before totalscore calculation : ", totalScore);
//       // Grade MCQ questions
...
//         }
//       }
//       console.log("after totalscore calculation : ", totalScore);
...
exports.postStartExam = async (req, res) => {
  try {
    ...
    // Grade MCQ questions
    if (mcqAnswers && Array.isArray(mcqAnswers) && mcqAnswers.length > 0) {
      for (const answer of mcqAnswers) {
        ...
        try {
          ...
          if (question) {
            // Compare as strings
            const selectedOption = answer.selectedOption.toString().trim();
